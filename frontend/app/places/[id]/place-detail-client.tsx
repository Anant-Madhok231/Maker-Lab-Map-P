...
export function PlaceDetailClient({ placeId }: PlaceDetailClientProps) {
  ...
  const activeOrigin = userLocation || addressPoint;
  const activeOriginLabel = userLocation ? "your pin" : addressPoint ? "your address" : null;
  const exactDistance = activeOrigin
    ? roundedMiles(distanceMiles(activeOrigin, destination))
    : null;
  const drivingUrl = googleDirectionsUrl({
    destination,
    mode: "driving",
    origin: activeOrigin || startAddress.trim() || null,
  });
  ...
  async function useAddressStart() {
    const trimmed = startAddress.trim();
    if (!trimmed) {
      setLocationStatus("Enter a starting address first.");
      return;
    }

    setLocationStatus("Looking up that starting address…");
    ...
    setLocationStatus(
      point
        ...
        : "Could not read that address yet. Drive and Walk can still open it in Google Maps.",
    );
  }
            ...
            <div className="border-t border-white/10 bg-white/5 p-7 lg:border-l lg:border-t-0">
              ...
              <div className="mt-6 rounded-[22px] border border-white/10 bg-white/5 p-4">
                <div className="grid gap-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ffb85c]">
                    Starting address
                  </label>
                  <input
                    className="rounded-full border border-white/10 bg-white px-4 py-2 text-sm font-bold text-[#172a20] outline-none placeholder:text-[#7b847e]"
                    onChange={(event) => setStartAddress(event.target.value)}
                    placeholder="Type your address"
                    value={startAddress}
                  />
                  <button
                    className="rounded-full border border-white/15 px-4 py-2 text-xs font-black text-white transition hover:-translate-y-0.5 hover:bg-white/10"
                    onClick={useAddressStart}
                    type="button"
                  >
                    Use this address
                  </button>
                ...
                <button
                  className="mt-3 w-full rounded-full bg-white px-4 py-2 text-xs font-black text-[#172a20] transition hover:-translate-y-0.5"
                ...
                <p className="mt-3 text-xs leading-5 text-[#c4d1c8]">
                  {exactDistance
                    ? `From ${activeOriginLabel || "your start"}: ${exactDistance}`
                    : locationStatus || "Enter an address or use your exact pin."}
                </p>
